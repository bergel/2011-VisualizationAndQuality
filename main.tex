...
This has been empirically reported by Sommerville~\cite{Somm00a} and Davis~\cite{Davi95a} who estimate that the cost of software maintenance accounts for 50\% to 75\% of the overall cost of a software system. Moreover, Corbi~\cite{Corb89a} reported that during maintenance, professionals spend at least half of their time analyzing software to understand it. 

Numerous tools have been produced to assist developers in their analysis. Unfortunately analysis tools live in complete isolation from each other, thus missing opportunities for sharing a common model, vocabulary, conventions and communication mediums. 

In this article we argue that, based on anecdotal evidences, simple visualizations can drastically improve the performance of actions related to the general notion of software quality, including execution time reduction, increased test coverage, better documentation and reduced component coupling. We describe four contextual tools  that intensively use visualization to communicate analysis result.

%Resorbing execution bottlenecks
%Assessing test coverage
%Assessing software documentation
...
%\sd{what is the claim? how relevant is the story behind mondrian? what make you believe that it can be generalized? 200 classes is nothing in terms of size?}
...
%Profiling Behavioral Blueprint
...
%\sd{the message of the paper cannot be that you did something on mondrian.}The upper part shows the coverage of the hierarchy of Version 2.2 of Mondrian.  Only 73\% of the methods are executed when running the unit tests.
...
%Designing a graphical library such as Mondrian is a difficult task in which the root of the graphical element class hierarchy is often bloated and complex. 
...
%. Version 2.70 of Mondrian contains 1,966 methods for which only 296 are commented (15\%). One immediate problem we faced when commenting source code, is to find what are the most relevant methods to comment first. Not all of these nearly 2,000 methods have the same relevance for a users. \sd{People like jm jezequel talk about multimillion line programs}. \sd{We do not care about the size of mondrian. What would be much much much better is ok the size of the script: with a correct engine you can get in 15 lines of code }

%Pharo does not come with a tool dedicate to comfortably write and browse code documentation. Using the standard tool to write code is the only way to read and write documentation as comment in the source code. The limitation of this way to document application code is the lack of global context. 

\myparagraph{Documentation blueprint}
If a method is complex and intensively used it is reasonable to properly comment it. We use this hypothesis as heuristic to guide our documentation effort. The \emph{documentation blueprint} uses a polymetric views to visually assess the necessity for documentation based on the complexity and the existing collaboration of methods.

%aims at guiding the commenting effort. The heuristic explored in this visualization is: if a method has a complex definition and is collaborating with many classes then it is a good candidate for being documenting in priority.
