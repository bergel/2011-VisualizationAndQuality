...
%I went over the paper yesterday. The major problem is that there is no story :).
%
%You cannot sell the visualizations because we have basically no validation and because most of them were already sold. You need to sell another story. I see two possibilities:
%
%1. "we need better technical tools"
%- the problem is that the current tools are not good enough to support important engineering scenarios
%- in this case the case studies should focus on the what these scenarios are, and provide evidence that supporting it them can bring improvements
%- thus, the sections would be named around the scenarios, not around the solutions
%- the current visualizations are then presented as examples of possible solutions
%
%2. "technical evolution is challenging"
%- the problem is that systems need to evolve not just because of functional requirements, but also because of non-functional requirements and technical issues
%- in this case, the actual visualizations would become secondary and we would mainly focus on the experience and difficulties that were encountered
%
...
\newcommand{\Title}{Contextual Tools: A Case for Visualization}
...
\sd{what is the goal of the paper?
For me software quality is vague and we could just gets killed for that.
You can quality model....}
\end{abstract}
...
This has been empirically verified\sd{reported - because I do not think sommerville did anything} by Sommerville~\cite{Somm00a} and Davis~\cite{Davi95a} who estimate that the cost of software maintenance accounts for 50\% to 75\% of the overall cost of a software system. Moreover, Corbi~\cite{Corb89a} reported that during maintenance, professionals spend at least half of their time analyzing software to understand it. 

% Why is the problem a problem?


% What's the surprising Idea?
We believe that one of the reasons for this dramatic situation is the tools that we commonly use to build software. Most of them absolutely lack adequate abstraction to represent program structure and execution, keys to understand all the implications associated to a software. For example, source code is still considered as the universal medium from which static properties can be extracted from and runtime stack frame evolution remains the uncontested oracle for finding execution bottleneck \sd{do you really mean that existing tools on runtime analysis do not exist?
reis, wim de pauw???}. Unfortunately, this has not changed during the last few decades.
...
Mondrian uses a fine grain \sd{grained?} version numbering schema: it comprises more than 500 successive different versions. This article is illustrated with a description of 11 versions of Mondrian.

\sd{what is the claim? how relevant is the story behind mondrian? what make you believe that it can be generalized?
200 classes is nothing in terms of size?}
\sd{what is the message to IEEE Software: idea: In this article we present based on anedoctal evidences that simple visualizations can drastically improve the performance of actions related to the general notion of software quality (speed, coverage...)}

\sd{Contextual and adequate visualizations are the point of the paper.}
\sd{Mondrian is just a use case. }\sd{In this article we show that with simple visualizations (based for example on Polymetric views) and some metrics it is possible to build tools that provide contextual and important feedback.}
...
\sd{this message should come earlier.}
\sd{I imagine that most people will not read the code so what is the message?}
...
We then increase the test coverage by writing tests for methods that are complex and depend on many other methods. \sd{this sentence is a bit out of context}
...
Our effort to increase the test coverage and remove dead code in the layout class hierarchy is summarized in the above part of \figref{TestBlueprintEvolution}. 

\sd{the message of the paper cannot be that you did something on mondrian.
}The upper part shows the coverage of the hierarchy of Version 2.2 of Mondrian.  Only 73\% of the methods are executed when running the unit tests.
%Version 2.2
% 21 methods, 1 red
% 10 methods, 7 red
% 13 methods, 2 red
...
\fig{}{0.8}{TestBlueprintEvolution}{Increase of the test coverage of the layout class hierarchy. \sd{no shadow} \sd{I do not see any green}}
...
The experience realized on Mondrian with \emph{test blueprint} did not solely consist\sd{s} in writing new unit tests. It also has given an opportunity to rethink crucial parts of the software. A class is visually represented as a set of interconnected methods. This representation conveys an intuitive sense of the inner complexity. Reducing this complexity has also been the topic of our experiments.
...
\sd{Nobody cares about the fact that Mondrian was, is or will be. They care that tools can be defined that are more adequate to their tasks.}

The importance of assessing and increasing code test coverage has been recognized according to the large number of available tools. We have compared test blueprint with \emph{all} \sd{most of} the available test coverage tools, independently of the considered programming language. None of them provide the combination of metrics and visual patterns offered by test blueprint.

\sd{the contextual and adequate feedback is present with a simple box and metrics}
...
Mondrian is a large \sd{no small} application which has a poorly documented source code. Version 2.70 of Mondrian contains 1,966 methods for which only 296 are commented (15\%). One immediate problem we faced when commenting source code, is to find what are the most relevant methods to comment first. Not all of these nearly 2,000 methods have the same relevance for a users. \sd{People like jm jezequel talk about multimillion line programs}. \sd{We do not care about the size of mondrian. What would be much much much better is ok the size of the script: with a correct engine you can get in 15 lines of code }
...
We have undertaken a significant effort to document Mondrian. Version 2.78 contains 1,972 methods with 482 commented methods (24\%). The right-hand side of \figref{DocumentationBlueprintEvolution} gives the blueprint for this version of the hierarchy. \sd{we do not care about the effort}
...
Version 2.89 is the result of a re-architectural effort. The number of bad dependencies has been reduced to 46. The number of good dependencies increased to 26. Version 2.89 contains 21 packages.  In the average, each package has 2.1 bad dependencies. \sd{encore une fois pareille. We do not care about that.}
...
The proportion of bad dependencies per package has been reduced by 65\% between Versions 2.80 and 2.89. \sd{same here... we do not really care -- it can be a good anecdcote but no more than that.}
...
%\fig{}{0.9}{Retrospective}{Number of added and open issues. \sd{do not care}}
...
%This article presents the major steps we went through at increasing the quality of Mondrian, a software large of more than 180 classes and 2,000 methods.
